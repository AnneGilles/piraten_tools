...
\frame{   
  ...
  \begin{itemize}
    ...
    \item <4-> Hilfe beim Seiten bearbeiten und formatieren: \\ \hfill \url{https://wiki.piratenpartei.de/Hilfe:Seite_bearbeiten}
    \item <5-> weitere Hilfen auf der Wiki-Startseite unter Wiki-Einstieg
...
\url{mailto:marburg@piratenpartei-hessen.de}
  \begin{itemize}
    ...
    \item <4-> keine Anh\"ange senden!
    \item <5-> besser: Datei (zB.\,ins Wiki) hochladen, Link senden
    \item <6-> Zitierstil (ToDo: Beispiel)
    \item <7-> Ordner anlegen, Filter anlegen (n\"achste Folie)
%%     \item <8-> 
   \end{itemize}
}

\frame{
  \frametitle{Thunderbird: Ordner \& Filter erstellen}
          \ifthenelse{\boolean{makeHandoutWithoutGraphics}}{}{\begin{textblock}{0}(6,4.5)
               \includegraphics<1>[scale=.4]{img/thunderbird-new-folder}
	       \end{textblock}}
          \ifthenelse{\boolean{makeHandoutWithoutGraphics}}{}{\begin{textblock}{0}(6,4.5)
               \includegraphics<2>[scale=.4]{img/thunderbird-extras}
	       \end{textblock}}
          \ifthenelse{\boolean{makeHandoutWithoutGraphics}}{}{\begin{textblock}{0}(4,5)
               \includegraphics<3>[scale=.3]{img/thunderbird-filter}
	       \end{textblock}}
          \ifthenelse{\boolean{makeHandoutWithoutGraphics}}{}{\begin{textblock}{0}(.3,6)
               \includegraphics<4>[scale=.4]{img/thunderbird-filter2}
	       \end{textblock}}
  \begin{itemize}
    \item <1-> Ordner erstellen (zb.\, per Rechtsklick auf Posteingang)
    \item <2-> Men\"u: Extras -- Filter
    \item <3-> Liste der Filter
    \item <4-> Filter einstellen
   \end{itemize}
}
...
\begin{enumerate}
    ...
    \item <5-> evtl vMB zur Best\"atigung durch alle Piraten?
\end{enumerate}
}
